...
{
    if (verbosity > 0)
        clog << format(gettext("Betrete Rekursionstiefe %1% (%2% × %3% Pixel)\n")) % n % img->width % img->height;
    if (img->noTransparentPixels())
    {
        if (verbosity > 0)
            clog << format(gettext("Keine transparenten Pixel in Tiefe %1%\n")) % n;
    }
    else
    {
    ...
    if (verbosity > 0)
        clog << format(gettext("Verlasse Rekursionstiefe %1%\n")) % n;
    return true;
}
...
int main(int argc, char** argv)
{
    ...
    while ((c = getopt(argc, argv, "o:hnvq")) != -1)
    {
        ...
        case 'o':
            ...
            break;
        case 'h':
            cout << gettext("    panofill -o AUSGABE [-h -n -v -q] EINGABE\n\n");
            cout << gettext("panofill ist ein Programm zur automatischen Vervollständigung sphärischer\n"
                            "360°·180°-Panoramabilder, das die Eigenschaften dieser Projektion\n"
                            "berücksichtigt.\n\n");
            cout << gettext("-h  Diesen Hilfetext ausgeben und das Programm beenden\n"
                            "-n  Nächster-Nachbar-Interpolation verwenden\n"
                            "-v  Mehr Statusinformationen anzeigen (kann mehrfach angegeben werden)\n"
                            "-q  Keine Statusinformationen anzeigen\n\n");
            return 0;
        case 'n':
            interpolator = 0;
            break;
    ...
    if (iname == NULL)
    {
        cerr << gettext("Keine Eingabedatei angegeben\n\n");
        return 1;
    }
    if (oname == NULL)
    {
        cerr << gettext("Keine Ausgabedatei angegeben\n\n");
        return 1;
    }
    ...
    if (img->data == NULL)
    {
        cerr << gettext("Fehler beim Laden des Bildes\n\n");
        return 1;
    }
    if (img->onlyTransparentPixels())
    {
        cerr << gettext("Das Bild ist vollständig transparent\n\n");
        return 1;
    }
    complete(img);
